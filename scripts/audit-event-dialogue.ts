...
async function main() {
  ...
  const eventQuests = catalog.quests.filter(
    (quest) => quest.category === "event",
  );
  let totalRaw = 0;
  let totalResolved = 0;
  ...
  for (const quest of eventQuests) {
    ...
    for (const [speaker, count] of counts) {
      ...
      if (characterIds.length > 0) {
        ...
        unresolvedSpeakers.set(
          speaker,
          (unresolvedSpeakers.get(speaker) ?? 0) + count,
        );
      }
    }
    totalRaw += questRaw;
    ...
    if (questRaw > 0 && questResolved === 0) {
      questsAllUnresolved.push(
        `${quest.encoreStoryId} ${quest.nameZh} (${questRaw} lines)`,
      );
    }
